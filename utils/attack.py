...
def main():
    ...
    optimized_w_path_selected = f"results/{config.wandb['wandb_init_args']['name']}/optimized_w_iter_0.pt"

    # print('optimized_w_path_selected',optimized_w_path_selected)
    # exit()
    if os.path.exists(optimized_w_path_selected) == False:
        ...
        #Log initial vectors
        if config.logging:
            os.mkdir(f"results/{config.wandb['wandb_init_args']['name']}")
    ...
    #     Path("results").mkdir(parents=True, exist_ok=True)
        # init_w_path = f"results/{config.wandb['wandb_init_args']['name']}/init_w_{run_id}.pt"
    ...
    for i in range(math.ceil(w.shape[0] / batch_size)):
        optimized_w_path = f"results/{config.wandb['wandb_init_args']['name']}/optimized_w_batch_{i}.pt"
        ...
        # imgs = optimization.synthesize(w_batch, num_ws)
        # save_images(imgs,'results/images','{}_s'.format(i))
        ...
        if os.path.exists(optimized_w_path) == False: #False
            ...
            print('w_batch_optimized',w_batch_optimized.shape)
            # Collect optimized style vectors
        # save images
        # imgs = optimization.synthesize(w_batch_optimized, num_ws)
        # save_images(imgs,'results/images','{}_e'.format(i))
